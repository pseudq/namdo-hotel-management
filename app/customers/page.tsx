...
export default function CustomersPage() {
  ...
  const filteredPastGuests = pastGuests.filter((guest) => guest.name.toLowerCase().includes(searchTerm.toLowerCase()))
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="space-y-4">
        <Collapsible open={isCurrentExpanded} onOpenChange={setIsCurrentExpanded} className="border rounded-lg">
          <CollapsibleTrigger className="flex items-center justify-between w-full p-4 hover:bg-muted/50">
            <h2 className="text-lg font-medium">Khách đang ở</h2>
            <ChevronDown
              className={`h-4 w-4 transition-transform ${isCurrentExpanded ? "transform rotate-180" : ""}`}
            />
          </CollapsibleTrigger>
          <CollapsibleContent>
            <Table>
        ...
        <Collapsible open={isPastExpanded} onOpenChange={setIsPastExpanded} className="border rounded-lg">
          <CollapsibleTrigger className="flex items-center justify-between w-full p-4 hover:bg-muted/50">
            <h2 className="text-lg font-medium">Khách đã ở</h2>
            <ChevronDown className={`h-4 w-4 transition-transform ${isPastExpanded ? "transform rotate-180" : ""}`} />
          </CollapsibleTrigger>
          <CollapsibleContent>
            <Table>
              <TableHeader>
