  ...
  const handleToggleStar = (notificationId: number) => {
    setNotificationList((prev) =>
      prev.map((notification) =>
        notification.id === notificationId ? { ...notification, isStarred: !notification.isStarred } : notification,
      ),
    )
  }
          ...
          >
            ...
            <button
              ...
              className={`p-1 rounded-md hover:bg-muted ${
                notification.isStarred ? "text-yellow-500" : "text-muted-foreground"
              }`}
            >
            ...
            <span className="text-sm text-muted-foreground whitespace-nowrap">{notification.timeDisplay}</span>
          </div>
        ))}
      </div>
    </div>
