...
export default function ReceiptList() {
  ...
  const filteredReceipts = useMemo(() => {
    if (!searchTerm || (searchFilter !== "all" && searchFilter !== "receipts")) {
      return receipts
    }
  ...
  return (
    <div className="border rounded-lg overflow-hidden">
        ...
        <Table>
          <TableHeader>
            <TableRow>
              ...
              <TableHead className="hidden md:table-cell">Số CCCD</TableHead>
              <TableHead>Trạng thái</TableHead>
              <TableHead className="hidden md:table-cell">Ngày tạo</TableHead>
              <TableHead className="hidden md:table-cell">Khuyến mãi</TableHead>
            ...
            {currentItems.length > 0 ? (
              currentItems.map((receipt) => (
                <TableRow key={receipt.id}>
                  ...
                  <TableCell className="max-w-[120px] truncate">{receipt.name}</TableCell>
                  <TableCell className="hidden md:table-cell">{receipt.idNumber}</TableCell>
                  <TableCell>
                    <span
                  ...
                  <TableCell className="hidden md:table-cell">{receipt.date}</TableCell>
                  <TableCell className="hidden md:table-cell">{receipt.discount.toLocaleString()}</TableCell>
      ...
      <div className="flex flex-wrap items-center justify-between px-4 py-2 border-t gap-2">
        <div className="text-sm">Tổng cộng {filteredReceipts.length} hóa đơn</div>
        ...
        <div className="text-sm font-medium">Tổng doanh thu: {totalRevenue.toLocaleString()} VNĐ</div>
      </div>
    </div>
  )
}
